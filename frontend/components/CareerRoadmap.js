...
function CurrentRoadmapView({ roadmap, userRole }) {
  return (
    <div className="space-y-8">
      ...
      {roadmap.positions_history && roadmap.positions_history.length > 0 && (
        <CareerHistoryTimeline
          positionsHistory={roadmap.positions_history}
          employmentInfo={roadmap.employment_info}
          employeeStatistics={roadmap.employee_statistics}
        />
      )}
...
// ============================================================
// CAREER HISTORY TIMELINE - Past positions at PSA
// ============================================================

function CareerHistoryTimeline({
  positionsHistory,
  employmentInfo,
  employeeStatistics,
}) {
  ...
  // Calculate years at PSA
  const calculateYearsAtPSA = () => {
    if (employeeStatistics?.years_of_service !== undefined) {
      return employeeStatistics.years_of_service.toFixed(1);
    }
    if (!employmentInfo?.hire_date) return "N/A";
    const hireDate = new Date(employmentInfo.hire_date);
    const now = new Date();
    const years = (now - hireDate) / (1000 * 60 * 60 * 24 * 365.25);
    return years.toFixed(1);
  };

  // Sort positions by start date (oldest first)
  const sortedPositions = [...positionsHistory].sort((a, b) => {
  ...
  return (
    <div className="glass-card border border-white/55 px-8 py-8">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h3 className="text-2xl font-bold text-slate-900 mb-2">
            📚 Your Career History at PSA
          </h3>
          <p className="text-slate-600">
            Your complete career progression journey
          </p>
        </div>
        <div className="glass-chip px-4 py-2 flex items-center gap-2">
          <Award className="w-4 h-4 text-indigo-500" />
          <span className="text-sm font-medium text-slate-700">
            {calculateYearsAtPSA()} years at PSA
          </span>
        </div>
      </div>
            ...
            <div key={index} className="relative">
              ...
              <div className="flex gap-6">
                ...
                <div className="flex-1 min-w-0">
                  <div className="glass-panel border border-white/50 px-6 py-5 hover:shadow-xl transition-shadow duration-300 bg-white/70">
                    {/* Role header */}
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <div className="glass-chip px-3 py-2 flex items-center gap-2 text-sm">
                          ...
                          <span className="font-medium text-slate-700">
                            {formatDate(position.period?.start)} -{" "}
                            {formatDate(position.period?.end)}
                          </span>
                        </div>
                    ...
                    {/* Focus areas */}
                    {position.focus_areas &&
                      position.focus_areas.length > 0 && (
                        <div className="mb-4">
                    ...
                    {/* Key skills */}
                    {position.key_skills_used &&
                      position.key_skills_used.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-widest text-slate-500 mb-2">
                            Key Skills Utilized
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {position.key_skills_used.map(
                              (skill, skillIndex) => (
                                <span
                                  key={skillIndex}
      ...
      {/* Summary Stats */}
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-8 pt-6 border-t border-white/20">
        <div className="glass-panel border border-white/40 p-5 text-center bg-white/50">
        ...
        <div className="glass-panel border border-white/40 p-5 text-center bg-white/50">
          ...
          <p className="text-lg font-semibold text-slate-900">
            {formatDate(
              sortedPositions[sortedPositions.length - 1]?.period?.start
            )}
          </p>
        </div>
        <div className="glass-panel border border-white/40 p-5 text-center bg-white/50">
        ...
        <div className="glass-panel border border-white/40 p-5 text-center bg-white/50">
          ...
          <p className="text-3xl font-bold text-emerald-600">
            {employeeStatistics?.tenure_percentile !== undefined
              ? `${employeeStatistics.tenure_percentile}%`
              : "N/A"}
          </p>
        ...
        <div className="glass-panel border border-white/40 p-5 text-center bg-white/50">
          ...
          <p className="text-3xl font-bold text-cyan-600">
            {employeeStatistics?.promotion_speed_percentile !== undefined
              ? `${employeeStatistics.promotion_speed_percentile}%`
              : "N/A"}
          </p>
